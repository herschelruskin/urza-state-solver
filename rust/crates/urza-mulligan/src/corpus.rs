...
use crate::{
    EVALUATED_HAND_RECORD_VERSION, EvaluatedHandRecord, ExactWinRate, HAND_FEATURE_SCHEMA_VERSION,
    ...
    MulliganReport, MulliganStage, ObjectivePreference, PregameContext, SampledDecisionConfidence,
};
...
impl EvaluatedHandCorpus {
    ...
    pub fn entries(
        &self,
    ) -> impl Iterator<Item = (&EvaluatedHandSampleId, &EvaluatedHandRecord)> {
        self.entries.iter()
    }
    ...
    pub fn insert_record(
        ...
        if self.entries.contains_key(&sample) {
...
fn summarize_component(
    cluster_ordinal: u32,
    component: &[usize],
    samples: &[(EvaluatedHandSampleId, &EvaluatedHandRecord, NormalizedHandFeatures)],
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidCurrentSevenSize(actual) => {
                write!(formatter, "current mulligan hand must contain seven cards, got {actual}")
            }
            ...
            Self::FeatureDrift(which) => {
                write!(formatter, "stored {which} features do not match card identities")
            }
            Self::IncompatibleConfiguration { .. } => {
                write!(formatter, "corpus records must share one evaluation configuration")
            }
            Self::EmptyFeatureVector => write!(formatter, "cannot normalize an empty feature vector"),
...
mod tests {
    ...
    fn transparent_single_link_grouping_is_stage_local_and_unlabeled() {
        ...
        let mut corpus = EvaluatedHandCorpus::new();
        for (world, record) in [(0, all_islands), (1, near), (2, far)] {
            corpus
                .insert_record(
                    EvaluatedHandSampleId::new(
                        RootSeed::from_u64(99),
                        WorldId(world),
                        MulliganStage::InitialSeven,
                    ),
                    record,
