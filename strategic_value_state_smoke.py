...
def test_hidden_order_and_seed_do_not_fragment_value_identity():
    ...
    b = replace(a, library=("Swan Song", "Island", "Mox Opal", "Sol Ring"), rng_root_seed=999)
...
def test_reporting_history_and_legacy_flags_do_not_fragment_base_value():
    ...
    assert key(a) == key(b)
    assert key(replace(a, won=True, win_family="Chrome Dome")) == key(
        replace(a, won=True, win_family="Top + Reality Chip")
    )
...
def test_perm_provenance_is_ignored_but_refund_credit_is_not():
    common = dict(name="Grinding Station", tapped=True, sick=False, knack_granted=True)
    a = base_state(battlefield=(Perm(**common, knack_source="Banishing Knack", instance_tag=1),))
    b = base_state(battlefield=(Perm(**common, knack_source="Retraction Helix", instance_tag=999),))
    assert key(a) == key(b)
    credited = base_state(battlefield=(Perm(**common, producer_urza_ready=True),))
    no_credit = base_state(battlefield=(Perm(**common, producer_urza_ready=False),))
...
def test_profiler_measures_collapse_without_changing_states():
    a = base_state(rng_root_seed=1)
    b = replace(a, library=("Mox Opal", "Swan Song", "Sol Ring", "Island"), rng_root_seed=2)
...
def test_profiler_denominator_ignores_shuffle_epoch_only():
    state = base_state()
    profiler = StrategicKeyProfiler()
    profiler.observe(state, InformationState(shuffle_epoch=1))
    profiler.observe(state, InformationState(shuffle_epoch=99))
    summary = profiler.summary()
    assert summary["concrete_unique"] == 1
    assert summary["concrete_information_unique"] == 1
    assert summary["strategic_unique"] == 1
...
def main():
    tests = [
        ...
        test_profiler_denominator_ignores_shuffle_epoch_only,
